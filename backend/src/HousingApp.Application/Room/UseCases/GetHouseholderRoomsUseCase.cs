...
public class GetHouseholderRoomsUseCase(IRoomRepository roomRepository, IStorageService storageService)
    : IGetHouseholderRoomsUseCase
{
    public async Task<Result<List<RoomHouseholderDto>>> ExecuteAsync(string userId)
    {
        ...
        List<RoomHouseholderDto> roomHouseholderDtoList =
        [
            .. roomHouseholderEntities.Select(r => new RoomHouseholderDto(
                r.Id,
                r.Name,
                ...
                ImageRoomUrls:
                [.. r.ImageRoomUrls.Select(imageKey => storageService.GeneratePresignedDownloadUrl(imageKey))]
            ))
        ];
