...
{
    public async Task<string> UploadAsync(Func<Stream> openStream, string fileName, string contentType,
        StorageType storageType, string entityId,
    ...
    {
        return $"{category.ToString().ToLower()}/{entityId}/{Guid.NewGuid():N}_{Path.GetFileName(fileName)}";
    }
}
